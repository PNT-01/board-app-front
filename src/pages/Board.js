...
const Board = () => {
    ...
    const findById = useCallback(async () => {
        try {
            const response = await axios.get(`http://211.188.51.104:9090/boards/${id}`, {
                headers: {
    ...
    const deleteById = useCallback(async () => {
        try {
            const response = await axios.delete(`http://211.188.51.104:9090/boards/${id}`, {
                headers: {
                    Authorization: `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`
                }
            });
    ...
    const imageLoader = (file) => {
        ...
        reader.onload = (e) => {
            ...
            if(file.name.toLowerCase().match(/(.*?)\.(jpg|png|jpeg|gif|svg|bmp)$/)) {
                ...
                // 정적인 파일들을 src폴더나 public 폴더에 둘 다 위치시킬 수 있는데
                // 파일을 가져오는 방식이 다르다.
                // src 폴더에 정적 파일을 위치시켰을 경우에는 import 구문으로 파일을 가져와야 한다.
    ...
    const modify = useCallback(async (formData) => {
        try {
            const response = await axios.patch('http://211.188.51.104:9090/boards', formData, {
                headers: {
                    Authorization: `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`
                }
            });
            ...
            <Grid container style={{marginTop: '3%', textAlign: 'center'}}>
                ...
                <Grid item
                    ...
                    <TextField name='regdate'
                               ...
                               fullWidth
                               size='small'
                               value={board != null ? board.regdate.substring(0, board.regdate.indexOf('T')) : ''}
                                ...
                                <input type='file'
                                       ...
                                       <img width='150px'
                                            ...
                                            src={boardFile.filetype === 'image'
                                                ? `http://kr.object.ncloudstorage.com/bitcamp73/${boardFile.filepath}${boardFile.filename}`
